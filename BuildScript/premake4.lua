...
solution("Log")
	...
	project ("Log")
		...
		configuration "Debug"
			flags { "Unicode" }
			flags { "Symbols" }
			objdir (working_dir .. "Temp" .. "/Log" .. "/Debug")
			targetdir (working_dir .. "Build" .. "/Log" .. "/Debug")
			if (premake_ver == 5) then optimize "Debug" end

		configuration "Release"
			flags { "Unicode" }
			flags { "Optimize" }
			objdir (working_dir .. "Temp" .. "/Log" .. "/Release")
			targetdir (working_dir .. "Build" .. "/Log" .. "/Release")
			if (premake_ver == 5) then optimize "Full" end
	
	------------------------------------------------------------------
	-- "Log Test One" library project
	------------------------------------------------------------------
	project ("Tests")
		language "C++"
		kind "ConsoleApp"
